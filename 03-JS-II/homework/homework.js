...
function tablaDelSeis(){
  ...
  //Escribe tu código aquí   
  var array = [];
  for (let mltdo = 0; mltdo < 11; mltdo++) {
    array.push( 6 * mltdo );
  }
    return array;
}
...
function tieneTresDigitos(numero){
  ...
  //Escribe tu código aquí
  var ntstr = (numero + '');
  var newstr= ntstr.length
  if (newstr === 3) {return true;
  } else {return false;
  }
}
...
function doWhile(numero) {
  ...
  // numero = 20 >>> numero= (numero) 20 + 5 >>>rtdo= 0 + 25 >>>>rtdo es igual a 20 + 40 NO rtdo = 25
  let rtdo = numero;
  let sum = 1;
  do { rtdo = rtdo + 5;
      sum = sum + 1;
